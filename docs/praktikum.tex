...
4. Privacy amplification & Kunci akhir $=$ SHA-1(bit). Menghapus informasi parity yang sempat bocor. \\
\bottomrule
\end{longtable}
...
\draw[dashed] (a0) -- ($(a0)+(0,-6.2)$) coordinate (aEnd);
\draw[dashed] (b0) -- ($(b0)+(0,-6.2)$) coordinate (bEnd);
% step labels (left)
...
% reconciliation loop
\node[draw,dashed,fit/.style={},font=\scriptsize] {};
...
\end{tikzpicture}
\caption{Alur protokol SKG: yang dikirim hanya posisi indeks dan parity, bukan nilai RSSI.}
\end{figure}
...
mahasiswa-skg/
  kelompokN/
    alice/   (laptop)  alice.py, skg_common.py, pcg32.py, net.py, alice.csv
    bob/     (ESP32)   bob_esp32/{bob_esp32.ino, skg.h, synced_bob.h}, bob.csv
  docs/              praktikum.tex
\end{lstlisting}
...
https://espressif.github.io/arduino-esp32/package_esp32_index.json
  ...
  \item Buka \texttt{kelompokN/bob/bob\_esp32/bob\_esp32.ino}, isi WiFi:
...
arduino-cli compile --fqbn esp32:esp32:esp32 kelompokN/bob/bob_esp32
arduino-cli upload  --fqbn esp32:esp32:esp32 -p COM_ESP32 kelompokN/bob/bob_esp32
arduino-cli monitor -p COM_ESP32 -c baudrate=115200
\end{lstlisting}

\subsection{C. Alice (Laptop)}
Edit \texttt{kelompokN/alice/alice.py}:
...
\begin{lstlisting}
# Laptop (Alice) -- contoh kelompok 1 (Aulia skenario 1)
...
\begin{enumerate}[noitemsep]
  ...
  \item Mengapa privacy amplification (SHA-1) diperlukan setelah rekonsiliasi?
